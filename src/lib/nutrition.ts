...
function months(ageYears: number, ageMonths: number) {
...
function stageFromAge(breed: BreedKey, totalMonths: number): LifeStage {
  ...
  if (totalMonths < adultThreshold) return 'puppy'
  if (totalMonths >= seniorThreshold) return 'senior'
  return 'adult'
}

export function computeNutrition(input: NutritionInput): NutritionOutput {
  const { breed, weightKg, ageYears, ageMonths, activity, neuter, goal } = input
  const totalMonths = months(ageYears, ageMonths)
  const lifeStage = stageFromAge(breed, totalMonths)
  const idealWeight = (BREEDS[breed].idealWeightRangeKg[0] + BREEDS[breed].idealWeightRangeKg[1]) / 2
  const kgForRER = goal === 'maintain' ? Math.max(1, weightKg) : Math.max(1, idealWeight)
  ...
  // Nutrient targets per 1000 kcal
